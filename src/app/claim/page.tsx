...
function ClaimForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Find the business by slug if prefilled. We do this so the claim row
    // links to the actual business; if there's no match, we still record the
    // claim with a null business_id and the admin can resolve it manually.
    ...
    if (error) {
      ...
      setState({
        kind: 'error',
        message:
          'Something went wrong submitting your claim. Please try again, or email us directly if it keeps failing.',
      })
      return
    }

    // Fire email notifications (admin + owner confirmation). Best-effort \u2014
    // a successful DB write is the primary success criterion.
    fetch('/api/email/claim', {
      ...
      // eslint-disable-next-line no-console
      console.error('[claim] email notification failed:', err)
    })
  ...
  if (state.kind === 'success') {
    return <SuccessView businessName={state.businessName} email={email} />
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-7" noValidate>
      ...
      >
        <input
          ...
          className={inputClassName(fieldErrors.businessName)}
          required
        />
      </Field>
      ...
      <Field
        ...
        hint="Optional. Tell us about your sustainability practices, certifications, or anything else that would help us verify your claim."
      >
        <textarea
          id="message"
          rows={4}
      ...
      <div className="max-w-2xl mx-auto px-6 py-12 md:py-16">
        <header className="mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-semibold tracking-tight text-stone-900 mb-4">
            Claim your business on Candella.
          </h1>
          <p className="text-stone-600 leading-relaxed">
            Own or manage a business that's already on Candella? Claim your
            listing to update your details, add photos, and get a verified
            owner badge. We verify every claim before granting access.
          </p>
        </header>
  ...
  return (
    <div className="py-10 md:py-16">
      ...
      <div className="text-stone-700 leading-relaxed space-y-4 mb-8">
        <p>
          Your claim for <span className="font-medium">{businessName}</span> is in our queue. We verify every
          claim by hand to make sure listings stay genuinely owner-managed. We've sent a confirmation
          to <span className="font-medium">{email}</span> — check your inbox.
        </p>
        <p>
          We'll usually be in touch within a couple of days. If we need to verify ownership another
          way (a quick DM from your business account, for example), we'll let you know.
        </p>
      </div>
